...
for ii = 1:size(samples,1)
    ...
    files = dir([samples(ii).folder filesep samples(ii).name filesep '*_ch1_Simple Segmentation.h5']); % gives unique samples
    nFiles = size(files,1);
    colonies = colonyS;
    ...
    for jj = 1:nFiles
        ...
        colonies(jj) = calculateRadialProfile(colonies(jj), meta, bins); %
        %fixed cell 
...
for ii = 1:meta.nChannels
    subplot(2,2,ii);
    errorbar(xValues(1:nValues), radialProfile_avg.dapiNormalized.mean(ii,:), radialProfile_avg.dapiNormalized.std(ii,:));
...
%%
% std error dapi normalized, all channels except dapi
figure; hold on;
for ii = 2:meta.nChannels
...
xlabel('Distance from center (mum)'); ylabel('Intensity (a.u.)');
legend(meta.channelNames(2:meta.nChannels));
