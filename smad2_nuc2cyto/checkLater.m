...
pixel_um = 0.62; % pixel to micron conversion factor. 
...
for condition1 = conditions(1)
    ...
    rA_colonies_dapi = zeros(numel(bins)-1, nColonies);
    % radial average of nuclear pixel profiles in the dapi channel.
    ...
    for ii = 1:nColonies
        ...
        [rA_colonies0(:,ii,:), nPixels(:,ii)] = radialAverage_ch2dapi_oneColony(nonDapiImages, dapiImage, ...
            masks{ii}, colonyMasks{ii}, bins_pixel, neighbourhoodSize_pixel);
        
        [rA_colonies_dapi(:,ii), ~] = radialAverage_dapi_oneColony(dapiImage, masks{ii}, colonyMasks{ii}, bins_pixel, neighbourhoodSize_pixel);
    ...
    save(outputFile, 'colonyMasks', 'rA_colonies_dapi', 'rA_colonies0', 'nPixels', 'bins', '-append');
    %save(outputFile, 'rA_colonies_dapi', '-append');
end
%%
