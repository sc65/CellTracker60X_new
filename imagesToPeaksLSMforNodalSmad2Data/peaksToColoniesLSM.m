function [colonies, groups, peaks]=peaksToColoniesLSM(matfile,mm,ts)
...
if  coltype == 0  % analysis for the circular colonies data;
    ...
    %Make colony structure for the alphavol algorithm
    ...
    % The data in colonies is somehow messed up. ---SC
    % Put absolute coordinates instead of relative coordinates. 
    for ii = 1:length(colonies)
        cellstouse=allinds==ii;
        colonies(ii).data = alldat(cellstouse,:);
    end
